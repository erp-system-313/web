...
export const useLead = (id: number) => {
  ...
  useEffect(() => {
    ...
    crmService.getLead(id)
      .then(setLead)
      .catch(() => setLead(fallbackLeads().find(l => l.id === id) || fallbackLeads()[0]))
...
const fallbackDashboard = (): CrmDashboardStats => ({
  totalLeads: 24,
  conversionRate: 32,
...
export const useCRMDashboard = () => {
  ...
  useEffect(() => {
    ...
    crmService.getDashboard()
      .then(setStats)
      .catch(() => setStats(fallbackDashboard()))
      .finally(() => setLoading(false));
  }, []);
