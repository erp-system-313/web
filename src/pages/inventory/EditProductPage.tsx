...
function buildCategoryTree(categories: Category[]): {
  value: number;
  title: string;
  children?: { value: number; title: string }[];
}[] {
  ...
  return parents.map((parent) => {
    ...
    return {
      ...
      children:
        children.length > 0
          ? children.map((c) => ({ value: c.id, title: c.name }))
          : undefined,
    };
  });
}
...
export const EditProductPage: React.FC = () => {
  ...
  const onBasicSubmit = (data: BasicInfoData) => {
    ...
    saveProduct(
      ...
      },
      {
        ...
        reorderLevel: fallback?.reorderLevel ?? 0,
      },
    );
  };
    ...
    saveProduct(
      {
        ...
        categoryId: fallback?.categoryId ?? 0,
      },
      data,
      {
        currentStock: fallback?.currentStock ?? 0,
  ...
  return (
    <FormCard title="Edit Product" backPath="/inventory/products">
