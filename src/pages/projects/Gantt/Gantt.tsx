...
export const Gantt: React.FC = () => {
  ...
  const { sortedItems, totalDays, dateAxis, maxHours } = useMemo(() => {
    if (!ganttItems || !project)
      return { sortedItems: [], totalDays: 0, dateAxis: [], maxHours: 1 };
  ...
  const stageNames = [
    ...new Set(sortedItems.map((i) => i.stageName).filter(Boolean)),
  ] as string[];
        ...
        {sortedItems.length === 0 ? (
          ...
          <>
              ...
              <div className={styles.timelineArea}>
                ...
                <div className={styles.rows}>
                  {sortedItems.map((item) => {
                    const projStart = project?.dateStart
                      ? dayjs(project.dateStart)
                      : dayjs();
                    ...
                    if (item.startDate) {
                      const startPos = Math.max(
                        0,
                        dayjs(item.startDate).diff(projStart, "day"),
                      );
                      const endPos = item.dueDate
                        ? dayjs(item.dueDate).diff(projStart, "day")
                        : totalDays;
                      leftPct = (startPos / totalDays) * 100;
                      widthPct = Math.max(
                        2,
                        ((endPos - startPos) / totalDays) * 100,
                      );
                    } else {
                      const pos = item.dueDate
                        ? dayjs(item.dueDate).diff(projStart, "day")
                        : 0;
                      const lp = Math.max(0, (pos / totalDays) * 100);
                      const wp = Math.max(
                        8,
                        ((item.estimatedHours || 1) / maxHours) * 60,
                      );
                      leftPct = Math.max(0, lp - wp);
                      widthPct = Math.min(wp, lp);
                    }
                    ...
                    return (
                      <div key={item.id} className={styles.taskRow}>
                        <div className={styles.taskNameCol}>
                          ...
                          {item.stageName && (
                            <Tag
                              color={getStageColor(item.stageName)}
            ...
            <div className={styles.legend}>
              ...
              {stageNames.map((name) => (
                <Tag key={name} color={getStageColor(name)}>
                  {name}
                </Tag>
              ))}
            </div>
